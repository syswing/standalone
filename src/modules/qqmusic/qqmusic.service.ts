...
export class QQMusicService {
	...
	async setUserCookie(cookies,response) {
		const res = await this.httpService.post('/user/setCookie', {
			data: cookies
		}).toPromise()
	...
	async getUserCookie(qqNo,cookies,response){
		const res = await this.httpService.get(`/user/getCookie?id=${qqNo}`,{
			headers:{
	...
	async userCookie(cookies){
		const res = await this.httpService.get(`/user/cookie`,{
			headers:{
				cookie:Object.keys(cookies).map(key => `${key}=${cookies[key]}`).join(';')
			}
		}).toPromise()
	...
	async refreshLogin(cookies) {
		const res = await this.httpService.get('/user/refresh',{
			headers:{
				cookie:Object.keys(cookies).map(key => `${key}=${cookies[key]}`).join(';')
			}
		}).toPromise()
	...
	async userDetail(userNo,cookies) {
		const res = await this.httpService.get(`/user/detail?id=${userNo}`,{
			headers:{
				cookie:Object.keys(cookies).map(key => `${key}=${cookies[key]}`).join(';')
			}
		}).toPromise()
	...
	async songlist(id,cookies){
		const res = await this.httpService.get(`/songlist?id=${id}`,{
			headers:{
				cookie:Object.keys(cookies).map(key => `${key}=${cookies[key]}`).join(';')
			}
		}).toPromise()
